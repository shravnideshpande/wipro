...
public class FlatBookingController {
	...
	public ResponseEntity<Void> acceptBooking(@PathVariable("id") int bookingId) {
        ...
        @PutMapping("/update")
        public ResponseEntity<FlatBooking> updateTenant(@Valid @RequestBody FlatBooking fbooking) {
        	fbookingService.updateFlatBooking( fbooking);
    		ResponseEntity<FlatBooking> responseEntity = new ResponseEntity<>(fbooking, HttpStatus.CREATED);
